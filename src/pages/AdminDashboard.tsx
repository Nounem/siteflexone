  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const totalPrice = gyms.reduce((sum, gym) => sum + (gym.pricePerMonth ?? 0), 0);
        const averagePrice = totalPrice / totalGyms;
        
        const cities = new Set(gyms.map(gym => gym.city));
        const citiesCount = cities.size;
        
        const topRatedGym = [...gyms].sort((a, b) => (b.rating ?? 0) - (a.rating ?? 0))[0];
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center">
            ...
            <div className="ml-5">
              ...
              {stats.topRatedGym && stats.topRatedGym.rating !== undefined 
              ? `${stats.topRatedGym.rating.toFixed(1)}/5` 
              : 'N/A'}
              </p>
            </div>
          </div>
              ...
              <div className="flex flex-col md:flex-row">
                ...
                <div className="w-full md:w-2/3 md:pl-6">
                  ...
                  <div className="flex items-center mb-4">
                    ...
                    <span>
                      {stats.topRatedGym?.rating !== undefined 
                        ? `${stats.topRatedGym.rating.toFixed(1)}/5 (${stats.topRatedGym.reviewCount ?? 0} avis)` 
                        : 'N/A'}
                    </span>
                  </div>
